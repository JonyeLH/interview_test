...
int main() {
	...
	//调用构造函数和析构函数只需知道函数地址即可，不与类型有关，编译器无需为此添加任何信息。对于普通函数也是一样，即使函数中声明了变量，也不占内存空间
	//若在析构函数标记为虚函数，则需要占据一定的空间。因为虚析构函数的调用会生成一个指针指向虚表，所以也就是对指针进行sizeof，32位系统指针占4位，64位占8位字节空间
