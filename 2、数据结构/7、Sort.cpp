...
int main() {
	...
	//选择排序	时间O(n^2) 空间O(1) 	数组不稳定、链表稳定	 n小时较好
	...
	//插入排序	时间O(n^2) 空间O(1) 	稳定算法		大部分已排序时较好
	...
	//希尔排序，插入排序的优化	  时间O(nlogn)	 空间O(1)	不稳定		性能与分组有关（gap）
	...
	//快速排序	时间O(nlogn)	空间O(nlogn)	不稳定		n大时较好
	...
	//归并排序递归实现		时间O(nlogn)	空间O(1)	稳定		n大时较好
